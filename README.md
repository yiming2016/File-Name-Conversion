...
- **Yandex**：官方 API
- **Bing 翻译**：微软免费接口，无需 Key
- **Papago**：Naver 官方 API（需要 Client ID / Client Secret）
- **百度翻译 / 火山翻译 / 小牛翻译 / 腾讯云**：国内翻译源，填密钥即可
- **AI 模型**：任意 OpenAI 兼容接口（DeepSeek、OpenAI、通义千问、智谱、Kimi、本地 Ollama、自定义等），每个预设渠道在“管理翻译源”里单独占一个块，和普通翻译渠道相同的布局；只需填接口地址 + Key + 模型名，模型名是下拉框，可直接输入或点“获取模型列表”自动读取，并支持一键查询 API Key 余额
...
- **跳过语言**：目标语言右侧可设置“跳过语言”（可多选），翻译时检测到属于这些语言的文件会直接跳过，不翻译、不改名；默认不跳过任何语言
...
- 设置窗口顶部页签分为两组：“常规”（通用 + 普通翻译渠道 + 自定义翻译源）和“AI”（各 AI 预设渠道），组名竖排显示在每组左侧；页签可长按拖动调整顺序（同一行内拖动，自动记忆）；“测试翻译”成功后对应页签变绿、失败变红；获取 API Key 的网址可直接点击用浏览器打开
...
- **AI 接口怎么填**：“管理翻译源 → AI”组里每个预设渠道（DeepSeek / OpenAI / 通义千问 / 智谱 / Kimi / 本地 Ollama / 自定义）都是一个独立块，已预填接口地址和模型名，只需填 API Key；本地 Ollama 可留空 Key，页内提供“下载Ollama”入口和适合翻译的推荐模型链接，点击可填入模型名并跳转对应下载地址。
